...
- 각 구현 완료 즉시 Playwright MCP를 사용하여 테스트 수행:
  - 브라우저 네비게이션 및 페이지 로딩 검증
  - UI 상호작용 및 사용자 플로우 테스트
  - API 호출 및 응답 검증
  - 에러 핸들링 및 엣지 케이스 테스트
- 테스트 실패 시 즉시 코드 수정 및 재테스트
- 모든 테스트 통과 확인 후에만 다음 단계로 진행
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - **각 API 엔드포인트 구현 후 Playwright MCP로 즉시 테스트**
  - **테스트 시나리오: 정상 케이스, 에러 케이스, 엣지 케이스 모두 검증**

- **Task 006: 인증 및 권한 시스템 구현**
  - 사용자 인증 시스템 구축
  - 권한 기반 접근 제어 구현
  - 보안 미들웨어 및 세션 관리
  - **각 인증 기능 구현 후 Playwright MCP로 즉시 테스트**
  - **테스트 시나리오: 로그인/로그아웃, 권한 검증, 세션 관리 등 모든 케이스 검증**
...
- [ ] 테스트 체크리스트에 다음 항목들이 포함되었는가?
  - [ ] 브라우저 네비게이션 및 페이지 로딩 검증
  - [ ] UI 상호작용 및 사용자 플로우 테스트
  - [ ] API 호출 및 응답 데이터 검증
  - [ ] 에러 핸들링 및 엣지 케이스 테스트
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 각 Task에 "구현 → 테스트 → 통과 확인" 절차가 명시되었는가?
- [ ] Phase 3에 통합 테스트 Task가 포함되었는가?
- [ ] 테스트 실패 시 수정 및 재테스트 프로세스가 명시되었는가?
